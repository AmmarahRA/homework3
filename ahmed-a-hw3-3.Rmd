...
```{r iv7table, include=TRUE, eval=TRUE, echo=False}
...
The IV regression from Table \@ref(tab:iv7table) shows that cigarette demand has a price elasticity of `r format(iv7$fit_log_price, big.mark=",")` which is inelastic. However, it is more inelastic compared to the previous regression without an instrument, suggesting that demand is even less responsive to price changes.
...
```{r coefftable, include=TRUE, eval=TRUE, echo=F}
...
```{r coefftab2, include=TRUE, eval=TRUE, echo=F}
